...
function HalfGiantTrebol({ activeStep, setActiveStep }) {
  const currentRotation = stepRotations[activeStep];

  return (
    <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 w-[650px] h-[650px] sm:w-[800px] sm:h-[800px] md:w-[920px] md:h-[920px] lg:w-[1020px] lg:h-[1020px] flex items-center justify-center select-none overflow-visible pointer-events-auto z-10">

      {/* Resplandor Verde Ambiental detrás de la hoja derecha inferior */}
      <motion.div
        animate={{
          scale: [1, 1.12, 1],
      ...
      {/* Trébol SVG Gigante Anclado en la Mitad Exacta del Borde Izquierdo */}
      <motion.div
            ...
            {steps.map((_, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* ÚNICAMENTE la hoja activa en la posición Derecho Inferior muestra el número */}
                  {isActive && (
                    <motion.text
                      x="0"
                      y="-130"
...
export default function Process() {
  ...
  return (
    <div id="process-section-wrapper" className="w-full relative">
      <section
        id="proceso"
        ref={containerRef}
        className="relative w-full h-screen bg-white text-carbon border-b border-carbon/10 select-none overflow-hidden"
      >
        ...
        <div className="absolute inset-0 pointer-events-none z-0">
          <motion.div
            ...
            className="absolute top-1/3 left-1/4 w-[40rem] h-[40rem] bg-trebol/20 rounded-full blur-[140px]"
          />
        </div>

        {/* Trébol Gigante Anclado a la Mitad del Borde Izquierdo */}
        <HalfGiantTrebol activeStep={activeStep} setActiveStep={setActiveStep} />

        {/* Contenido Principal a la Derecha del Trébol */}
        <div className="w-full h-full max-w-[1400px] mx-auto flex flex-col justify-center px-6 sm:px-12 md:pl-64 lg:pl-[420px] xl:pl-[480px] pr-6 md:pr-12 relative z-20 pt-12 md:pt-16 pb-10">

          {/* Encabezado del Título */}
          <div className="w-full mb-6 md:mb-8">
            <h2 className="text-3xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-tight text-carbon">
              Metodología <span className="text-trebol">Dinámica.</span>
            </h2>
          </div>

          {/* Tarjeta con Detalles del Paso Actual */}
          <div className="w-full relative min-h-[380px] md:min-h-[440px] flex items-center">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}
                className="w-full p-6 md:p-10 rounded-[2.5rem] bg-hueso/90 backdrop-blur-xl border border-neutral-200/90 shadow-[0_20px_50px_rgba(0,0,0,0.06)] relative overflow-hidden"
              >
                ...
                <div className="flex items-center justify-between mb-5 border-b border-neutral-200 pb-5">
                  <div className="flex items-center gap-4">
                    <span className="text-4xl md:text-6xl font-black text-trebol font-mono leading-none">
                      {currentStep.number}.
                    </span>
                    <div>
                      <h3 className="text-2xl md:text-4xl font-black tracking-tight text-carbon">
                        {currentStep.title}
                      </h3>
                  ...
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-trebol/10 border border-trebol/30 flex items-center justify-center text-trebol shrink-0 hidden sm:flex">
                    <StepIcon size={26} />
                  </div>
                </div>

                {/* Descripción del Paso */}
                <p className="text-base md:text-xl text-carbon/80 font-light leading-relaxed mb-6">
                  {currentStep.description}
                </p>

                {/* Tecnologías & Entregables */}
                <div className="space-y-3">
                  <div className="text-[11px] uppercase tracking-wider font-mono text-carbon/60">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {currentStep.tecnologias.map((tech, i) => (
                      ...
                      >
                        <CheckCircle2 size={13} className="text-trebol" />
                        {tech}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mt-6 pt-5 border-t border-neutral-200 flex items-center justify-between text-xs font-mono text-carbon/60">
